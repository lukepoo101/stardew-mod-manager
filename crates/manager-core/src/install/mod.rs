...
impl InstallPlan {
    pub fn verify_staged_content(&self, _staged_dir: &std::path::Path) -> Result<(), String> {
        // Transitional shim for legacy use_cases; full verification is in manager-infra StagedContentVerifier
        Ok(())
    }
}

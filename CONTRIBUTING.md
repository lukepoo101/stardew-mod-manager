...
This version is a Linux-only MVP. Keep changes focused on correctness, accessibility and the existing install/launch/remove workflow. New platforms, distribution integrations and automatic mod updates are outside the current scope.
...
- `manager-core`: models, manifest/dependency rules and use cases. Some existing use cases perform filesystem work; do not assume this crate is entirely pure.
- `manager-infra`: SQLite, archives, file locking, SMAPI installer, launcher and log reader.
- `apps/desktop`: React UI, mock backend and Tauri commands.
